...
- **Grid completeness:** every (bench, runtime, s, o) cell present, **5/5 invocations** captured. gc-pacing ran 23 benches × 25 cells in the original sweep; trunk ran 24 (it additionally builds `pplacer_testsuite`, which still fails on gc-pacing). alt_ergo×3 and jsoo were excluded in the original sweep but both **now build cleanly on all four runtimes** after two follow-up infrastructure fixes (cmdliner bump to v2.1.1 for jsoo, and a 9-line workaround in dolmen's `base.ml` for the OCaml-5.6-trunk typechecker bug that crashes alt-ergo) — see *Follow-ups* below. The (s, o) tables in this report reflect the original 22-bench set, since rerunning the full sweep across the larger bench set hasn't happened yet. No parse errors, no missing cells.
...
4. **jsoo now builds on all four runtimes** (5.4.1, d8bb46c, trunk-cfb30145, gc-pacing) after [`scripts/setup-monorepo.sh`](../../scripts/setup-monorepo.sh) bumped the vendored cmdliner from `1.3.0+dune` → `v2.1.1`. The previous failure was `Unbound module Arg.Completion` — jsoo upstream requires `cmdliner >= 2.0` but opam-monorepo had given us 1.3. Verified standalone: 28/28 benches build cleanly on each of 5.4.1 and d8bb46c with the new cmdliner; no other benches regress.
5. **alt-ergo now builds on all four runtimes** after patching [`duniverse/dolmen/src/typecheck/base.ml`](../../scripts/setup-monorepo.sh) (the patch is applied by `setup-monorepo.sh`). The crash was `Fatal error: exception Ctype.Unify(_)` — an OCaml 5.6-trunk typechecker bug in `Typecore.type_function`/`type_newtype` triggered by `term_app_chain` and `term_app_chain_ast` passing the same `(module Type)` (carrying two locally-abstract types `env` and `term`) to both `make_chain` and `map_chain`, where `map_chain` introduces its own locally-abstract type `t`. Inlining `map_chain`'s 9-line body at the two call sites sidesteps the nested-locally-abstract-type unification the typechecker mishandles. Worth reporting upstream to ocaml/ocaml — the trigger is small enough to minimise into a standalone repro.
6. **`pplacer_testsuite` on gc-pacing remains the only failing bench in the 4-runtime matrix** (28/28 on each of 5.4.1, d8bb46c, trunk-cfb30145; 27/28 on gc-pacing). Cause: `batteries.batGc.mli` records `Gc.stat` with the pre-PR shape, but the gc-pacing PR refactored the `Gc.stat` record. The 5.4.1 / d8bb46c / trunk pplacer builds all work — this is a real PR-side downstream consumer break that needs a `batteries` bump on the PR.
