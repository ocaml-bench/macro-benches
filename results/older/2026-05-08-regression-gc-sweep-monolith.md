...
## Intra-d8b tuning view — what's actionable from inside d8b

*Added 2026-05-14 as a follow-up to the
[2026-05-13 sweeps report](../latest/2026-05-13-regression-and-offheap-sweeps-monolith.md),
which introduced the cross-version vs intra-runtime distinction. This
section answers the production question — "I'm already on d8b, can I tune
to beat my current default?" — using the 5-08 data, and reflects against
the 5-13 conclusions on the overlapping cells.*

The version-effect tables above are all `d8b/5.4` ratios. For a production
user already on d8b, the more useful view is `d8b at cell / d8b at default`,
which says how much room is available *within d8b* by tuning GC params.
These views can disagree: a cell where d8b beats 5.4 may not actually be
fast in absolute terms; a cell where d8b loses ground to 5.4 may still be
the production-optimal pick because both runtimes improve there.

### Sweep A — (s, o) intra-d8b summary

Per bench: the d8b cell with the lowest wall vs d8b default, and the cell
with the lowest RSS vs d8b default. Gains are `% improvement over d8b
default at (s=262144, o=120)`.

| benchmark | d8b default wall / RSS | min-wall cell | wall gain | min-RSS cell | RSS gain |
|---|---|---|---|---|---|
| **liq_parse_typecheck** | 9.36s / 48 MB | `(s=1048576, o=150)` | **-27.0%** | `(s=1048576, o=100)` | **-32.5%** |
| **ocamlformat_rocq** | 1.79s / 295 MB | `(s=1048576, o=200)` | **-8.9%** | `(s=524288, o=80)` | -7.8% |
| **alt_ergo_fill** | 5.07s / 939 MB | `(s=524288, o=200)` | -8.5% | `(s=524288, o=80)` | -8.0% |
| **cpdf_scale** | 12.86s / 489 MB | `(s=1048576, o=200)` | -8.5% | `(s=524288, o=80)` | -13.8% |
| **cpdf_merge** | 2.16s / 363 MB | `(s=1048576, o=200)` | -8.3% | `(s=524288, o=80)` | -5.2% |
| **cpdf_squeeze** | 3.42s / 329 MB | `(s=1048576, o=200)` | -7.3% | `(s=65536, o=80)` | -12.2% |
| **ocamlc_self_compile** | 3.30s / 967 MB | `(s=1048576, o=200)` | -7.0% | `(s=1048576, o=80)` | -3.5% |
| **menhir_sysver** | 7.70s / 746 MB | `(s=524288, o=200)` | -6.6% | `(s=262144, o=80)` | -1.6% |
| **pplacer_testsuite** | 6.22s / 70 MB | `(s=1048576, o=120)` | -4.8% | `(s=262144, o=80)` | -2.8% |
| **liq_video_frames** | 4.67s / 517 MB | `(s=131072, o=200)` | -2.8% | `(s=262144, o=100)` | -3.6% |
| **devkit_stre** | 4.06s / 23 MB | `(s=1048576, o=120)` | -2.0% | `(s=524288, o=80)` | -1.9% |

The pattern is uniform across benches: **larger `s`, larger `o` is the
intra-d8b winning quadrant**. The biggest win is `liq_parse_typecheck` at
-27% wall and -32% RSS simultaneously — a strict-better cell. The
remaining benches sit at 5-9% wall improvements with various RSS trades.

### Sweep B — (M, m) intra-d8b summary

| benchmark | d8b default wall / RSS | min-wall cell | wall gain | min-RSS cell | RSS gain |
|---|---|---|---|---|---|
| **liq_video_frames** | 4.65s / 517 MB | `(M=176, m=25)` | **-37.2%** | `(M=11, m=400)` | -27.9% |
| **pplacer_testsuite** | 6.17s / 70 MB | `(M=176, m=200)` | -1.6% | `(M=88, m=200)` | -0.7% |
| **alt_ergo_fill** | 5.07s / 938 MB | `(M=88, m=50)` | -1.2% | `(M=11, m=100)` | -3.1% |
| **liq_parse_typecheck** | 9.38s / 49 MB | `(M=44, m=25)` | -0.9% | `(M=22, m=100)` | -1.4% |
| **devkit_stre** | 4.09s / 23 MB | `(M=176, m=50)` | -0.7% | `(M=176, m=400)` | -0.8% |
| **cpdf_scale** | 12.90s / 490 MB | `(M=22, m=200)` | -0.6% | `(M=88, m=25)` | -0.3% |
| **ocamlc_self_compile** | 3.29s / 968 MB | `(M=176, m=400)` | -0.6% | `(M=22, m=200)` | -0.2% |
| **ocamlformat_rocq** | 1.79s / 295 MB | `(M=176, m=25)` | -0.6% | `(M=88, m=200)` | -0.3% |
| **cpdf_merge** | 2.15s / 363 MB | `(M=44, m=50)` | -0.5% | `(M=176, m=100)` | -0.4% |
| **menhir_sysver** | 7.67s / 747 MB | `(M=176, m=25)` | -0.4% | `(M=11, m=50)` | -0.2% |
| **cpdf_squeeze** | 3.42s / 328 MB | `(M=176, m=50)` | -0.3% | `(M=44, m=50)` | -0.3% |

Only `liq_video_frames` has a meaningful intra-d8b gain on (M, m); the
rest are within ±1.6% of default — confirming the 5-08 cross-version
finding that **(M, m) is structurally inert except on the genuinely
off-heap-pacer-bound benchmark**.

For `liq_video_frames`: `M=176` gives -37% wall at the cost of +113% RSS;
`M=11` flips this to **+165% wall for -28% RSS**. The same Pareto
trade-off the cross-version report described, now visible in absolute
terms relative to d8b default.

### Highlighted heatmap: liq_parse_typecheck intra-d8b (s, o)

d8b default `(s=262144, o=120)`: **9.36s / 48 MB**

| s \ o | 80 | 100 | 120 **(def)** | 150 | 200 |
|---|---|---|---|---|---|
| **65536** | +0.7 / +0 | +5.5 / +9 | +7.1 / +1 | +9.3 / +4 | +12.0 / +13 |
| **131072** | -7.5 / +3 | -6.9 / +0 | +2.0 / -2 | +5.7 / +12 | +4.5 / +6 |
| **262144** **(def)** | -16.6 / -7 | -12.0 / -4 | +0.0 / +0 | +13.6 / +24 | +13.6 / +5 |
| **524288** | -26.6 / -27 | -22.5 / -27 | -10.1 / -11 | -7.1 / -9 | -3.6 / -3 |
| **1048576** | -26.6 / -29 | -26.7 / -33 | **-22.0 / -19** | **-27.0 / -28** | -26.8 / -7 |

The whole `s ≥ 524288` half of the grid is a -7 to -27% absolute
improvement over d8b default. This is the same shape the 5-13 sweep
extended to s=2097152 (where the gain reached -33%) — confirmed at this
sweep's narrower coverage.

### Reflection vs the 5-13 sweep

5-13's Sweep A re-ran (s, o) on the same regressed benches with a wider
grid (`s ∈ {131072, ..., 2097152}`, `o ∈ {40, ..., 200}` vs 5-08's
`s ∈ {65536, ..., 1048576}`, `o ∈ {80, ..., 200}`). The 4×4 cell overlap
on `(s ∈ {131072, 262144, 524288, 1048576}, o ∈ {80, 120, 150, 200})`
lets us cross-check each bench's intra-d8b min-wall cell across the
two runs:

| bench | 5-08 min-wall (gain) | 5-13 min-wall (gain) | agreement |
|---|---|---|---|
| liq_parse_typecheck | `(1048576, 150)` -27.0% | `(2097152, 200)` -33.3% | ✓ same direction; 5-13 extends the grid and finds -6pp deeper |
| ocamlformat_rocq | `(1048576, 200)` -8.9% | `(1048576, 200)` -9.4% | ✓ identical cell |
| alt_ergo_fill | `(524288, 200)` -8.5% | `(1048576, 200)` -8.5% | ✓ adjacent cell, same gain |
| cpdf_scale | `(1048576, 200)` -8.5% | `(2097152, 200)` -10.0% | ✓ same direction |
| cpdf_merge | `(1048576, 200)` -8.3% | `(2097152, 200)` -7.0% | ✓ same direction |
| cpdf_squeeze | `(1048576, 200)` -7.3% | `(2097152, 200)` -8.4% | ✓ same direction |
| ocamlc_self_compile | `(1048576, 200)` -7.0% | `(1048576, 200)` -6.7% | ✓ identical cell |
| menhir_sysver | `(524288, 200)` -6.6% | `(1048576, 200)` -6.2% | ✓ adjacent |
| pplacer_testsuite | `(1048576, 120)` -4.8% | `(2097152, 120)` -9.2% | ✓ same direction |
| liq_video_frames (=_full in 5-13) | `(131072, 200)` -2.8% | `(1048576, 200)` -0.6% | ✓ both "(s, o) inert" |
| devkit_stre | `(1048576, 120)` -2.0% | `(2097152, 200)` -3.7% | ✓ both small effect |

**11/11 benches agree on direction.** Where the runs differ in
magnitude, 5-13 typically finds a deeper win because its grid extends to
`s=2097152` (which the 5-08 grid stopped short of). The relative rankings
are essentially identical. The intra-d8b conclusions are robust across
the two independent sweeps:

- **liq_parse_typecheck** has the deepest intra-d8b win, ~27-33%
- **The cpdf_*, ocamlformat_rocq, ocamlc_self_compile, alt_ergo_fill,
  menhir_sysver group** all sit at -6 to -9% intra-d8b wall improvements
- **liq_video_frames and devkit_stre are (s, o)-inert** — only -2 to -3%
  available
- **Larger `s` and larger `o`** is the universal intra-d8b sweet-spot
  quadrant

The 5-13 sweep adds the new finding that **larger `s` neutralises
`zarith_pi`'s `o`-axis sensitivity** (intra-d8b -25% wall and -66% RSS
at `(2097152, 40)`). 5-08 didn't include zarith_pi, so this is genuinely
new.

For Sweep B, 5-08 swept `(M, m)` while 5-13 swept `(M, o)` — different
second axis. But the M-axis story is consistent across both:
**`liq_video_frames` intra-d8b improves dramatically with large M**.
5-08 saw `M=176, m=25` give -37% wall; 5-13 saw `M=250, o=150` give
-44%. Same direction, same magnitude band. The "5.5-beta has a hidden
M-knob for liquidsoap-style streaming" finding survives both runs and
both choices of second axis.

[ocaml/ocaml#14533](https://github.com/ocaml/ocaml/issues/14533) is
confirmed at the workload level: on a real liquidsoap-ffmpeg streaming
pipeline, `M=250` cuts CPU by 17% on Ryzen at the cost of +10% RSS — the
same intra-d8b trade visible across both synthetic sweeps.
